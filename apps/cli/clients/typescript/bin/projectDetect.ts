...
export const detectProjectType = async (projectDir: string): Promise<string> => {
  ...
  if (await fs.pathExists(packageJsonPath)) {
    ...
    const isBackend =
      ...
      (await fs.pathExists(path.join(projectDir, "server.js"))) ||
  ...
  const pythonFiles = await fs
    .readdir(projectDir)
    .catch(() => [])
    .then((files) => files.filter((f) => f.endsWith(".py")));
  ...
  if (isPythonProject) {
    ...
    detectedTypes.push({
      type: pythonType,
      category: frameworks.some((f) =>
        ["Django", "FastAPI", "Flask", "Tornado", "Pyramid", "AIOHTTP", "Sanic"].includes(f),
      )
        ? "backend"
        : "other",
      confidence: Math.min(confidence, 100),
  ...
  const rubyFiles = await fs
    .readdir(projectDir)
    .catch(() => [])
    .then((files) => files.filter((f) => f.endsWith(".rb")));
  ...
  if ((hasClientDir && hasServerDir) || (hasAppsDir && hasPackagesDir)) {
    ...
    if (hasAppsDir) {
      ...
      for (const app of appsDir) {
        ...
        if (
          appType.toLowerCase().includes("react") ||
          appType.toLowerCase().includes("vue") ||
          appType.toLowerCase().includes("angular") ||
          appType.toLowerCase().includes("front")
        ) {
    ...
    if (hasPackagesDir) {
      ...
      for (const pkg of packagesDir) {
        ...
        if (pkgType !== "Unknown") {
          if (
            pkgType.toLowerCase().includes("react") ||
            pkgType.toLowerCase().includes("vue") ||
            pkgType.toLowerCase().includes("angular") ||
            pkgType.toLowerCase().includes("front")
          ) {
