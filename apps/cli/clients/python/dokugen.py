...
def ask_yes_no(message):
    try:
        ...
        if answer is None: return "cancel"
    ...
    try:
        ...
        if template_url:
            ...
            if readme_exists and not args.overwrite:
                with open(readme_path, 'r', encoding='utf-8') as f: existing_content = f.read()
...
def cmd_update(args):
    ...
    try:
        ...
        generate_readme_remote(project_type, project_files, project_dir, existing_content, args.template, gemini_key)
...
def main():
    parser = argparse.ArgumentParser(description="Dokugen - AI Readme Generator")
    ...
    up_parser = subparsers.add_parser("update", help="Update auto-generated sections")
    up_parser.add_argument("--template", help="Use a custom GitHub repo readme template")
    
    docker_parser = subparsers.add_parser("compose-docker", help="Generate dockerfiles")
    ...
    if len(sys.argv) == 1:
        ...
        action = questionary.select(
            ...
            choices=[
                questionary.Choice("Generate README", value="generate"),
                questionary.Choice("Update README", value="update"),
