...
const getUserInfo = (): {
  ...
  try {
    ...
    console.log(
      chalk.yellow("⚠️ Git User Info not found. Using Defaults......")
    );
  }
...
const restoreReadme = async (): Promise<string | null> => {
  if (readmeBackup && currentReadmePath) {
    try {
      ...
      console.log(
        chalk.green("♻️ Original README content restored successfully")
      );
      return readmeBackup;
    } catch (error) {
  ...
  try {
    ...
    const response = await axios.post(
      `${backendDomain}/api/generate-readme`,
      {
        ...
        fullCode,
        userInfo,
        options: { includeSetup, includeContributionGuideLine, noEmojis },
        existingReadme,
        repoUrl,
        templateUrl,
      },
...
program
  ...
  .option("--no-emoji", "Generate README without emojis")
      ...
      if (readmeExists) {
        if (!options.overwrite) {
          ...
          if (overwrite === true) {
            ...
            console.log(
              chalk.yellow("⚠️ README update skipped (user selected No)")
            );
            return;
