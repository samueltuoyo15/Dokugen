      ...
      >
        <BrowserChrome title="~/project-orion">
          <div style={{ height: 640, backgroundColor: "#18181b", padding: "42px 48px" }}>
            <TerminalLine color="#a1a1aa">
              samuel@dev project-orion % <span style={{ color: "#fafafa" }}>{typed}</span>
              <span style={{ color: colors.yellow, opacity: frame % 18 < 9 ? 1 : 0 }}>▋</span>
            </TerminalLine>
            <div style={{ height: 28 }} />
            <TerminalLine style={{ opacity: ease(frame, 45, 10) }} color={colors.purpleSoft}>
              ◆ Dokugen is reading your codebase
            </TerminalLine>
            <div style={{ height: 18 }} />
            {[
              ...
              return (
                <TerminalLine key={label} style={{ opacity: visible }} color="#a1a1aa">
                  <span style={{ color: done ? colors.green : colors.yellow }}>{done ? "✓" : "·"}</span>
                  {"  "}
                  {label.padEnd(24, " ")}
...
const Callout = ({
  ...
  top,
  delay,
}: { title: string; detail: string; color: string; top: number; delay: number }) => {
  const frame = useCurrentFrame();
...
const DemoScene = () => {
  ...
  return (
    <Scene duration={210}>
      ...
      <Callout
        title="Smart updates"
        detail="Refresh generated sections. Keep every custom note."
        color={colors.green}
        top={212}
        delay={32}
      />
      <Callout
        ...
        detail="Turn the codebase into a readable architecture map."
        color={colors.purple}
        top={404}
        delay={70}
      />
      <Callout
        title="Quality commits"
        detail="Create Conventional Commits from the actual diff."
        color={colors.blue}
        top={596}
        delay={108}
      />
...
const FeatureTile = ({
  ...
  y,
  delay,
}: { title: string; command: string; color: string; x: number; y: number; delay: number }) => {
  const frame = useCurrentFrame();
